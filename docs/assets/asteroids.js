...
(function(Opal) {
  ...
  Opal.add_stubs(['$attr_reader', '$/', '$graphics', '$add', '$begin_fill', '$draw_polygon', '$<', '$-@', '$>', '$-', '$+', '$*', '$x=', '$y=', '$angle=', '$ensure_bounds', '$sin', '$deg_to_rad', '$math', '$cos', '$**', '$dir_x', '$dir_y', '$limit_speed', '$between', '$rnd', '$draw_circle', '$text', '$background_color=', '$stage', '$new', '$map', '$abs', '$times', '$physics_elapsed', '$time', '$down?', '$keyboard', '$input', '$speed_up', '$turn', '$text=', '$%', '$update', '$each', '$collision?', '$x', '$y', '$sqrt', '$state']);
  self.$require("app/asteroids"+ '/../' + "common");
  (function($base, $super) {
    function $SpaceShip(){};
  ...
  (function($base, $super) {
    ...
    Opal.defn(self, '$initialize', function(x, y) {
      ...
      self.dy = $rb_times($scope.get('Math').$sin($gvars.game.$math().$deg_to_rad(angle)), speed);
      self.graphics = $gvars.game.$add().$graphics(self.x, self.y);
      self.graphics.$begin_fill(16711680);
      return self.graphics.$draw_circle(0, 0, 20);
    });
  ...
  (function($base, $super) {
    ...
    def.active = def.space_ship = def.score = def.score_text = def.asteroids = nil;
    Opal.defn(self, '$create', function() {
      var $a, $b, TMP_1, self = this;
      if ($gvars.game == null) $gvars.game = nil;
      ...
      return self.asteroids = ($a = ($b = (10).$times()).$map, $a.$$p = (TMP_1 = function(){var self = TMP_1.$$s || this, $a, $b, x = nil, y = nil;
        if ($gvars.game == null) $gvars.game = nil;
        if ($gvars.size_x == null) $gvars.size_x = nil;
        if ($gvars.size_y == null) $gvars.size_y = nil;
      ...
      while ((($b = true) !== nil && (!$b.$$is_boolean || $b == true))) {
        ...
        if ((($b = $rb_gt($rb_plus(($rb_minus(x, $rb_divide($gvars.size_x, 2))).$abs(), ($rb_minus(y, $rb_divide($gvars.size_y, 2))).$abs()), 600)) !== nil && (!$b.$$is_boolean || $b == true))) {
          break;};};
        return $scope.get('Asteroid').$new(x, y);}, TMP_1.$$s = self, TMP_1), $a).call($b);
    });
    ...
    Opal.defn(self, '$update', function() {
      ...
      if ((($a = self.active) !== nil && (!$a.$$is_boolean || $a == true))) {
        } else {
        return nil
      };
      ...
      if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.RIGHT)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.space_ship.$turn(1.0, dt)};
      self.score = $rb_plus(self.score, dt);
      (($a = ["You survived %.3fs"['$%']([self.score])]), $b = self.score_text, $b['$text='].apply($b, $a), $a[$a.length-1]);
      self.space_ship.$update(dt);
      return ($a = ($b = self.asteroids).$each, $a.$$p = (TMP_2 = function(a){var self = TMP_2.$$s || this, $a;
        if (self.space_ship == null) self.space_ship = nil;
if (a == null) a = nil;
      a.$update(dt);
        if ((($a = self['$collision?'](self.space_ship, a)) !== nil && (!$a.$$is_boolean || $a == true))) {
