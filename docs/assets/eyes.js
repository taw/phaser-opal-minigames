...
(function(Opal) {
  ...
  Opal.add_stubs(['$/', '$*', '$min', '$between', '$rnd', '$+', '$-', '$graphics', '$add', '$begin_fill', '$line_style', '$draw_circle', '$rand_retina_color', '$to_i', '$%', '$sqrt', '$>', '$x=', '$y=', '$background_color=', '$stage', '$flatten', '$map', '$new', '$each', '$update', '$worldX', '$active_pointer', '$input', '$worldY', '$state']);
  self.$require("app/eyes"+ '/../' + "common");
  (function($base, $super) {
    function $Eye(){};
    var self = $Eye = $klass($base, $super, 'Eye', $Eye);

    var def = self.$$proto, $scope = self.$$scope;

    def.eyesize = def.x = def.y = def.eye = def.retina = def.max_eye_movement = nil;
    Opal.defn(self, '$initialize', function(cell_x, cell_y) {
      var self = this, cx = nil, cy = nil, max_eye_size = nil;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;
      if ($gvars.game == null) $gvars.game = nil;

      cx = $rb_divide($rb_times(cell_x, $gvars.size_x), 4);
      cy = $rb_divide($rb_times(cell_y, $gvars.size_y), 4);
      max_eye_size = $rb_divide([$gvars.size_x, $gvars.size_y].$min(), 4.0);
      self.eyesize = $gvars.game.$rnd().$between($rb_times(max_eye_size, 0.3), $rb_times(max_eye_size, 0.8));
      self.x = $gvars.game.$rnd().$between($rb_plus(cx, 50), $rb_minus($rb_plus(cx, $rb_divide($gvars.size_x, 4)), 50));
      self.y = $gvars.game.$rnd().$between($rb_plus(cy, 50), $rb_minus($rb_plus(cy, $rb_divide($gvars.size_y, 4)), 50));
      self.max_eye_movement = $rb_times(self.eyesize, 0.2);
      self.eye = $gvars.game.$add().$graphics(self.x, self.y);
      self.eye.$begin_fill(16777215);
      self.eye.$line_style(3, 0, 1);
      self.eye.$draw_circle(0, 0, self.eyesize);
      self.retina = $gvars.game.$add().$graphics(self.x, self.y);
      self.retina.$begin_fill(self.$rand_retina_color());
      return self.retina.$draw_circle(0, 0, $rb_times(self.eyesize, 0.5));
    });

    Opal.defn(self, '$rand_retina_color', function() {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;

      return ("%02x%02x%02x"['$%']([$gvars.game.$rnd().$between(140, 255), $gvars.game.$rnd().$between(90, 140), $gvars.game.$rnd().$between(90, 140)])).$to_i(16);
    });

    return (Opal.defn(self, '$update', function(mx, my) {
      var $a, $b, self = this, dx = nil, dy = nil, dl = nil;

      dx = $rb_minus(mx, self.x);
      dy = $rb_minus(my, self.y);
      dl = $scope.get('Math').$sqrt($rb_plus($rb_times(dx, dx), $rb_times(dy, dy)));
      if ((($a = $rb_gt(dl, self.max_eye_movement)) !== nil && (!$a.$$is_boolean || $a == true))) {
        dx = $rb_divide($rb_times(self.max_eye_movement, dx), dl);
        dy = $rb_divide($rb_times(self.max_eye_movement, dy), dl);};
      (($a = [$rb_plus(self.x, dx)]), $b = self.retina, $b['$x='].apply($b, $a), $a[$a.length-1]);
      return (($a = [$rb_plus(self.y, dy)]), $b = self.retina, $b['$y='].apply($b, $a), $a[$a.length-1]);
    }), nil) && 'update';
  })($scope.base, null);
  (function($base, $super) {
    function $MainState(){};
    ...
    Opal.defn(self, '$create', function() {
      ...
      return self.eyes = ($a = ($b = ($range(0, 3, false))).$map, $a.$$p = (TMP_1 = function(x){var self = TMP_1.$$s || this, $a, $b, TMP_2;
if (x == null) x = nil;
      return ($a = ($b = ($range(0, 3, false))).$map, $a.$$p = (TMP_2 = function(y){var self = TMP_2.$$s || this;
if (y == null) y = nil;
        return $scope.get('Eye').$new(x, y)}, TMP_2.$$s = self, TMP_2), $a).call($b)}, TMP_1.$$s = self, TMP_1), $a).call($b).$flatten();
    });

    return (Opal.defn(self, '$update', function() {
      var $a, $b, TMP_3, self = this;

      return ($a = ($b = self.eyes).$each, $a.$$p = (TMP_3 = function(eye){var self = TMP_3.$$s || this;
        if ($gvars.game == null) $gvars.game = nil;
if (eye == null) eye = nil;
      return eye.$update($gvars.game.$input().$active_pointer().$worldX(), $gvars.game.$input().$active_pointer().$worldY())}, TMP_3.$$s = self, TMP_3), $a).call($b);
    }), nil) && 'update';
