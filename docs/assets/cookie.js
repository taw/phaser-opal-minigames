...
(function(Opal) {
  ...
  self.$require("app/cookie"+ '/../' + "common");
  (function($base, $super) {
    function $MainState(){};
    ...
    Opal.defn(self, '$preload', function() {
      ...
      $gvars.game.$load().$image("cookie", "/images/cookie.png");
      $gvars.game.$load().$image("banana", "/images/banana.png");
      return $gvars.game.$load().$audio("coin", "/audio/balsa.mp3");
    });
    ...
    Opal.defn(self, '$update', function() {
      ...
      if ((($a = ((($b = $rb_lt(self.cookie.$x(), 60)) !== false && $b !== nil) ? $b : $rb_ge(self.cookie.$x(), $rb_minus($gvars.size_x, 60)))) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.dx = self.dx['$-@']()};
      if ((($a = ((($b = $rb_lt(self.cookie.$y(), 60)) !== false && $b !== nil) ? $b : $rb_ge(self.cookie.$y(), $rb_minus($gvars.size_y, 60)))) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.dy = self.dy['$-@']()};
    ...
    return (Opal.defn(self, '$create', function() {
      var $a, $b, TMP_1, $c, self = this, angle = nil, speed = nil;
      if ($gvars.game == null) $gvars.game = nil;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;
      ...
      (($a = [true]), $b = self.cookie, $b['$input_enabled='].apply($b, $a), $a[$a.length-1]);
      ($a = ($b = self.cookie.$events()).$on, $a.$$p = (TMP_1 = function(){var self = TMP_1.$$s || this, $a, $b;
        if (self.score == null) self.score = nil;
        if (self.cookie == null) self.cookie = nil;
        if (self.emitter == null) self.emitter = nil;
        if (self.coin == null) self.coin = nil;

      self.score = $rb_plus(self.score, 1);
        (($a = [self.cookie.$x()]), $b = self.emitter, $b['$x='].apply($b, $a), $a[$a.length-1]);
        (($a = [self.cookie.$y()]), $b = self.emitter, $b['$y='].apply($b, $a), $a[$a.length-1]);
        self.emitter.$start(true, 2000, nil, 1);
        return self.coin.$play();}, TMP_1.$$s = self, TMP_1), $a).call($b, "down");
      angle = $gvars.game.$math().$deg_to_rad($gvars.game.$rnd().$between(0, 360));
      speed = 200.0;
      self.dx = $rb_times($scope.get('Math').$cos(angle), speed);
      self.dy = $rb_times($scope.get('Math').$sin(angle), speed);
      self.emitter = $gvars.game.$add().$emitter(0, 0, 1000);
      self.emitter.$make_particles("banana");
      (($a = [-50]), $c = self.emitter, $c['$gravity='].apply($c, $a), $a[$a.length-1]);
      (($a = [$rb_times(-0.1, self.dx)]), $c = self.emitter.$minParticleSpeed(), $c['$x='].apply($c, $a), $a[$a.length-1]);
      (($a = [$rb_times(-0.1, self.dy)]), $c = self.emitter.$minParticleSpeed(), $c['$y='].apply($c, $a), $a[$a.length-1]);
      (($a = [$rb_times(-0.4, self.dx)]), $c = self.emitter.$maxParticleSpeed(), $c['$x='].apply($c, $a), $a[$a.length-1]);
      (($a = [$rb_times(-0.4, self.dy)]), $c = self.emitter.$maxParticleSpeed(), $c['$y='].apply($c, $a), $a[$a.length-1]);
      return self.emitter.$set_alpha(0.5, 0.8, 0);
    }), nil) && 'create';
