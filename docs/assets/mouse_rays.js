...
(function(Opal) {
  ...
  Opal.add_stubs(['$background_color=', '$stage', '$new', '$set_resolution', '$sprite', '$add', '$width=', '$height=', '$filters=', '$update', '$active_pointer', '$input', '$state']);
  self.$require("app/mouse_rays"+ '/../' + "common");
  self.$require("app/mouse_rays"+ '/../' + "shaders/mouse_rays");
  (function($base, $super) {
    function $MainState(){};
    ...
    Opal.defn(self, '$create', function() {
      ...
      (($a = ["0F8"]), $b = $gvars.game.$stage(), $b['$background_color='].apply($b, $a), $a[$a.length-1]);
      self.filter = (($scope.get('Phaser')).$$scope.get('Filter')).$new($gvars.game, nil, $scope.get('ShaderMouseRays'));
      self.filter.$set_resolution($gvars.size_x, $gvars.size_y);
      self.sprite = $gvars.game.$add().$sprite();
      (($a = [$gvars.size_x]), $b = self.sprite, $b['$width='].apply($b, $a), $a[$a.length-1]);
      (($a = [$gvars.size_y]), $b = self.sprite, $b['$height='].apply($b, $a), $a[$a.length-1]);
      return (($a = [[self.filter]]), $b = self.sprite, $b['$filters='].apply($b, $a), $a[$a.length-1]);
    });
    ...
    return (Opal.defn(self, '$update', function() {
      ...
      return self.filter.$update($gvars.game.$input().$active_pointer());
    }), nil) && 'update';
