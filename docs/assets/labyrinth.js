...
(function(Opal) {
  ...
  Opal.add_stubs(['$image', '$load', '$audio', '$map', '$to_proc', '$lines', '$strip', '$create', '$width=', '$height=', '$set', '$anchor', '$immovable=', '$body', '$/', '$sprite', '$add', '$enable', '$physics', '$group', '$enable_body=', '$size', '$[]', '$each_with_index', '$*', '$+', '$===', '$add_box', '$add_star', '$add_cat', '$chars', '$background_color=', '$stage', '$start_system', '$create_cursor_keys', '$keyboard', '$input', '$setup_map', '$collide', '$arcade', '$overlap', '$play', '$destroy', '$x=', '$down?', '$right', '$cat_speed', '$left', '$-@', '$velocity', '$y=', '$down', '$up', '$state', '$new']);
  self.$require("app/labyrinth"+ '/../' + "common");
  (function($base, $super) {
    function $MainState(){};
    ...
    def.map = def.boxes = def.box_x_size = def.box_y_size = def.stars = def.cat = def.cursors = nil;
    Opal.defn(self, '$preload', function() {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;

      $gvars.game.$load().$image("cat", "/images/cat_images/cat17.png");
      $gvars.game.$load().$image("green_box", "/images/green_box.png");
    ...
    Opal.defn(self, '$map', function() {
      ...
      return ((($a = self.map) !== false && $a !== nil) ? $a : self.map = ($b = ($c = "\n    ************\n    * x        *\n    *    **  ***\n    ****  *   x*\n    * x*  ******\n    * **       *\n    *     c  x *\n    ************\n    ".$strip().$lines()).$map, $b.$$p = "strip".$to_proc(), $b).call($c));
    });

    Opal.defn(self, '$add_box', function(x, y) {
      var $a, $b, self = this, box = nil;

      box = self.boxes.$create(x, y, "green_box");
      (($a = [self.box_x_size]), $b = box, $b['$width='].apply($b, $a), $a[$a.length-1]);
      (($a = [self.box_y_size]), $b = box, $b['$height='].apply($b, $a), $a[$a.length-1]);
      box.$anchor().$set(0.5);
      return (($a = [true]), $b = box.$body(), $b['$immovable='].apply($b, $a), $a[$a.length-1]);
    });

    Opal.defn(self, '$add_star', function(x, y) {
      var $a, $b, self = this, star = nil;

      star = self.stars.$create(x, y, "star");
      star.$anchor().$set(0.5);
      (($a = [$rb_divide(self.box_x_size, 4)]), $b = star, $b['$width='].apply($b, $a), $a[$a.length-1]);
      (($a = [$rb_divide(self.box_y_size, 4)]), $b = star, $b['$height='].apply($b, $a), $a[$a.length-1]);
      return (($a = [true]), $b = star.$body(), $b['$immovable='].apply($b, $a), $a[$a.length-1]);
    });

    Opal.defn(self, '$add_cat', function(x, y) {
      var $a, $b, self = this;
      if ($gvars.game == null) $gvars.game = nil;

      self.cat = $gvars.game.$add().$sprite(x, y, "cat");
      $gvars.game.$physics().$enable(self.cat, (((($scope.get('Phaser')).$$scope.get('Physics'))).$$scope.get('ARCADE')));
      self.cat.$anchor().$set(0.5);
      (($a = [$rb_divide(self.box_x_size, 2)]), $b = self.cat, $b['$width='].apply($b, $a), $a[$a.length-1]);
      return (($a = [$rb_divide(self.box_y_size, 2)]), $b = self.cat, $b['$height='].apply($b, $a), $a[$a.length-1]);
    });

    Opal.defn(self, '$setup_map', function() {
      var $a, $b, TMP_1, self = this;
      if ($gvars.game == null) $gvars.game = nil;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;

      self.boxes = $gvars.game.$add().$group();
      (($a = [true]), $b = self.boxes, $b['$enable_body='].apply($b, $a), $a[$a.length-1]);
      self.box_x_size = $rb_divide($gvars.size_x, self.$map()['$[]'](0).$size());
      self.box_y_size = $rb_divide($gvars.size_y, self.$map().$size());
      self.stars = $gvars.game.$add().$group();
      (($a = [true]), $b = self.stars, $b['$enable_body='].apply($b, $a), $a[$a.length-1]);
      return ($a = ($b = self.$map()).$each_with_index, $a.$$p = (TMP_1 = function(row, y){var self = TMP_1.$$s || this, $a, $b, TMP_2;
if (row == null) row = nil;if (y == null) y = nil;
      return ($a = ($b = row.$chars()).$each_with_index, $a.$$p = (TMP_2 = function(cell, x){var self = TMP_2.$$s || this, $a, sx = nil, sy = nil, $case = nil;
          if (self.box_x_size == null) self.box_x_size = nil;
          if (self.box_y_size == null) self.box_y_size = nil;
if (cell == null) cell = nil;if (x == null) x = nil;
        $a = [$rb_times(($rb_plus(x, 0.5)), self.box_x_size), $rb_times(($rb_plus(y, 0.5)), self.box_y_size)], sx = $a[0], sy = $a[1], $a;
          return (function() {$case = cell;if ("*"['$===']($case)) {return self.$add_box(sx, sy)}else if ("x"['$===']($case)) {return self.$add_star(sx, sy)}else if ("c"['$===']($case)) {return self.$add_cat(sx, sy)}else { return nil }})();}, TMP_2.$$s = self, TMP_2), $a).call($b)}, TMP_1.$$s = self, TMP_1), $a).call($b);
    });
    ...
    Opal.defn(self, '$create', function() {
      ...
      (($a = ["88F"]), $b = $gvars.game.$stage(), $b['$background_color='].apply($b, $a), $a[$a.length-1]);
    ...
    Opal.defn(self, '$cat_speed', function() {
      var self = this;
    ...
    return (Opal.defn(self, '$update', function() {
      ...
      $gvars.game.$physics().$arcade().$collide(self.cat, self.boxes);
      ($a = ($b = $gvars.game.$physics().$arcade()).$overlap, $a.$$p = (TMP_3 = function(c, s){var self = TMP_3.$$s || this;
        if (self.coin == null) self.coin = nil;
if (c == null) c = nil;if (s == null) s = nil;
      self.coin.$play();
        return s.$destroy();}, TMP_3.$$s = self, TMP_3), $a).call($b, self.cat, self.stars);
      (($a = [(function() {if ((($d = self.cursors.$right()['$down?']()) !== nil && (!$d.$$is_boolean || $d == true))) {
        return self.$cat_speed()
      } else if ((($d = self.cursors.$left()['$down?']()) !== nil && (!$d.$$is_boolean || $d == true))) {
        return self.$cat_speed()['$-@']()
        } else {
        return 0
      }; return nil; })()]), $c = self.cat.$body().$velocity(), $c['$x='].apply($c, $a), $a[$a.length-1]);
      return (($a = [(function() {if ((($d = self.cursors.$down()['$down?']()) !== nil && (!$d.$$is_boolean || $d == true))) {
        return self.$cat_speed()
      } else if ((($d = self.cursors.$up()['$down?']()) !== nil && (!$d.$$is_boolean || $d == true))) {
        return self.$cat_speed()['$-@']()
        } else {
        return 0
      }; return nil; })()]), $c = self.cat.$body().$velocity(), $c['$y='].apply($c, $a), $a[$a.length-1]);
    }), nil) && 'update';
