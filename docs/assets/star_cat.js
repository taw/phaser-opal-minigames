...
(function(Opal) {
  ...
  self.$require("app/star_cat"+ '/../' + "common");
  (function($base, $super) {
    function $Star(){};
  ...
  (function($base, $super) {
    ...
    Opal.defn(self, '$update', function(dt) {
      ...
      self.phase = $rb_plus(self.phase, dt);
      (($a = [$rb_times(($rb_plus(0.5, $rb_times(0.1, $scope.get('Math').$sin($rb_times(self.phase, 2))))), $gvars.size_x)]), $b = self.cat, $b['$x='].apply($b, $a), $a[$a.length-1]);
      return (($a = [$rb_times(($rb_plus(0.5, $rb_times(0.05, $scope.get('Math').$cos($rb_times(self.phase, 2))))), $gvars.size_y)]), $b = self.cat, $b['$y='].apply($b, $a), $a[$a.length-1]);
    });
  ...
  (function($base, $super) {
    ...
    var self = $TrailEmitter = $klass($base, $super, 'TrailEmitter', $TrailEmitter);

    var def = self.$$proto, $scope = self.$$scope;

    def.emitter = def.phase = nil;
    Opal.defn(self, '$initialize', function() {
      var $a, $b, self = this;
      if ($gvars.game == null) $gvars.game = nil;
      ...
      self.emitter.$make_particles("star2");
      (($a = [0]), $b = self.emitter, $b['$gravity='].apply($b, $a), $a[$a.length-1]);
      (($a = [110]), $b = self.emitter.$maxParticleSpeed(), $b['$x='].apply($b, $a), $a[$a.length-1]);
      (($a = [90]), $b = self.emitter.$minParticleSpeed(), $b['$x='].apply($b, $a), $a[$a.length-1]);
      (($a = [10]), $b = self.emitter.$maxParticleSpeed(), $b['$y='].apply($b, $a), $a[$a.length-1]);
      (($a = [-10]), $b = self.emitter.$minParticleSpeed(), $b['$y='].apply($b, $a), $a[$a.length-1]);
      return self.emitter.$set_alpha(0.2, 0.5, 0);
    });

    return (Opal.defn(self, '$update', function(dt, cat) {
      var $a, $b, self = this;

      (($a = [cat.$x()]), $b = self.emitter, $b['$x='].apply($b, $a), $a[$a.length-1]);
      (($a = [cat.$y()]), $b = self.emitter, $b['$y='].apply($b, $a), $a[$a.length-1]);
      self.phase = $rb_plus(self.phase, dt);
      if ((($a = $rb_gt(self.phase, 0.05)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.phase = $rb_minus(self.phase, 0.05);
        return self.emitter.$start(true, 5000, nil, 1);
        } else {
        return nil
      };
    }), nil) && 'update';
  ...
  (function($base, $super) {
    ...
    def.nyan_music = def.cat = def.stars = def.emitter = nil;
    Opal.defn(self, '$preload', function() {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;

      $gvars.game.$load().$image("cat", "/images/cat_images/cat4.png");
      $gvars.game.$load().$image("star", "/images/star-icon.png");
      $gvars.game.$load().$audio("nyan", "/audio/nyan.mp3");
      return $gvars.game.$load().$image("star2", "/images/star2.png");
    });
    ...
    Opal.defn(self, '$create', function() {
      ...
      self.stars = ($a = ($b = (50).$times()).$map, $a.$$p = (TMP_1 = function(i){var self = TMP_1.$$s || this;
if (i == null) i = nil;
      return $scope.get('Star').$new($rb_divide(i, 50.0))}, TMP_1.$$s = self, TMP_1), $a).call($b);
      self.emitter = $scope.get('TrailEmitter').$new();
    ...
    return (Opal.defn(self, '$update', function() {
      ...
      self.cat.$update(dt);
      ($a = ($b = self.stars).$each, $a.$$p = (TMP_2 = function(star){var self = TMP_2.$$s || this;
if (star == null) star = nil;
      return star.$update(dt)}, TMP_2.$$s = self, TMP_2), $a).call($b);
      return self.emitter.$update(dt, self.cat);
    }), nil) && 'update';
