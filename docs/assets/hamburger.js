...
(function(Opal) {
  ...
  Opal.add_stubs(['$sprite', '$add', '$set', '$anchor', '$height=', '$width=', '$reset_hamburger_position', '$y=', '$+', '$y', '$*', '$x=', '$x', '$between?', '$-', '$between', '$rnd', '$deg_to_rad', '$math', '$cos', '$sin', '$/', '$clamp', '$image', '$load', '$text', '$background_color=', '$stage', '$map', '$new', '$times', '$**', '$<', '$physics_elapsed', '$time', '$each', '$update', '$down?', '$keyboard', '$input', '$move', '$text=', '$burger_eaten?', '$state']);
  self.$require("app/hamburger"+ '/../' + "common");
  (function($base, $super) {
    function $Hamburger(){};
    var self = $Hamburger = $klass($base, $super, 'Hamburger', $Hamburger);

    var def = self.$$proto, $scope = self.$$scope;

    def.graphics = def.speed_x = def.speed_y = nil;
    Opal.defn(self, '$initialize', function() {
      var $a, $b, self = this;
      if ($gvars.game == null) $gvars.game = nil;

      self.graphics = $gvars.game.$add().$sprite(0, 0, "hamburger");
      self.graphics.$anchor().$set(0.5, 0.5);
      (($a = [64]), $b = self.graphics, $b['$height='].apply($b, $a), $a[$a.length-1]);
      (($a = [64]), $b = self.graphics, $b['$width='].apply($b, $a), $a[$a.length-1]);
      return self.$reset_hamburger_position();
    });

    Opal.defn(self, '$update', function(time) {
      var $a, $b, self = this;
      if ($gvars.size_y == null) $gvars.size_y = nil;
      if ($gvars.size_x == null) $gvars.size_x = nil;

      ($a = self.graphics, $a['$y=']($rb_plus($a.$y(), $rb_times(self.speed_x, time))));
      ($a = self.graphics, $a['$x=']($rb_plus($a.$x(), $rb_times(self.speed_y, time))));
      if ((($a = ($b = self.graphics.$y()['$between?'](32, $rb_minus($gvars.size_y, 32)), $b !== false && $b !== nil ?self.graphics.$x()['$between?'](32, $rb_minus($gvars.size_x, 32)) : $b)) !== nil && (!$a.$$is_boolean || $a == true))) {
        return nil
        } else {
        return self.$reset_hamburger_position()
      };
    ...
    Opal.defn(self, '$reset_hamburger_position', function() {
      var $a, $b, self = this, angle = nil, speed = nil;
      if ($gvars.game == null) $gvars.game = nil;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;

      (($a = [$gvars.game.$rnd().$between(32, $rb_minus($gvars.size_x, 32))]), $b = self.graphics, $b['$x='].apply($b, $a), $a[$a.length-1]);
      (($a = [$gvars.game.$rnd().$between(32, $rb_minus($gvars.size_y, 32))]), $b = self.graphics, $b['$y='].apply($b, $a), $a[$a.length-1]);
      angle = $gvars.game.$math().$deg_to_rad($gvars.game.$rnd().$between(0, 360));
      speed = $gvars.game.$rnd().$between(22, 200);
      self.speed_x = $rb_times($scope.get('Math').$cos(angle), speed);
      return self.speed_y = $rb_times($scope.get('Math').$sin(angle), speed);
    });
  ...
  (function($base, $super) {
    function $Smiley(){};
    var self = $Smiley = $klass($base, $super, 'Smiley', $Smiley);

    var def = self.$$proto, $scope = self.$$scope;

    def.graphics_smiley = nil;
    Opal.defn(self, '$initialize', function() {
      var $a, $b, self = this;
      if ($gvars.game == null) $gvars.game = nil;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      ...
      self.graphics_smiley = $gvars.game.$add().$sprite($rb_divide($gvars.size_x, 2), $rb_divide($gvars.size_y, 2), "smiley");
      self.graphics_smiley.$anchor().$set(0.5, 0.5);
      (($a = [80]), $b = self.graphics_smiley, $b['$height='].apply($b, $a), $a[$a.length-1]);
      return (($a = [80]), $b = self.graphics_smiley, $b['$width='].apply($b, $a), $a[$a.length-1]);
    });

    Opal.defn(self, '$move', function(direction_x, direction_y, time) {
      var $a, $b, self = this;
      if ($gvars.game == null) $gvars.game = nil;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;

      ($a = self.graphics_smiley, $a['$x=']($rb_plus($a.$x(), $rb_times($rb_times(time, direction_x), 300))));
      (($a = [$gvars.game.$math().$clamp(self.graphics_smiley.$x(), 40, $rb_minus($gvars.size_x, 40))]), $b = self.graphics_smiley, $b['$x='].apply($b, $a), $a[$a.length-1]);
      ($a = self.graphics_smiley, $a['$y=']($rb_plus($a.$y(), $rb_times($rb_times(time, direction_y), 300))));
      return (($a = [$gvars.game.$math().$clamp(self.graphics_smiley.$y(), 40, $rb_minus($gvars.size_y, 40))]), $b = self.graphics_smiley, $b['$y='].apply($b, $a), $a[$a.length-1]);
    });

    Opal.defn(self, '$x', function() {
      var self = this;

      return self.graphics_smiley.$x();
    });

    return (Opal.defn(self, '$y', function() {
      var self = this;

      return self.graphics_smiley.$y();
    }), nil) && 'y';
  })($scope.base, null);
  (function($base, $super) {
    function $MainState(){};
    ...
    def.smiley = def.hamburgers = def.score = def.score_text = nil;
    Opal.defn(self, '$preload', function() {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;

      $gvars.game.$load().$image("hamburger", "/images/hamburger.png");
      return $gvars.game.$load().$image("smiley", "/images/smiley.png");
    });
    ...
    Opal.defn(self, '$create', function() {
      ...
      self.score = 0;
      self.score_text = $gvars.game.$add().$text(10, 10, "", $hash2(["fontSize", "fill", "align"], {"fontSize": "16px", "fill": "#FA4659", "align": "center"}));
      (($a = ["#FFC47F"]), $b = $gvars.game.$stage(), $b['$background_color='].apply($b, $a), $a[$a.length-1]);
      self.hamburgers = ($a = ($b = (15).$times()).$map, $a.$$p = (TMP_1 = function(){var self = TMP_1.$$s || this;

      return $scope.get('Hamburger').$new()}, TMP_1.$$s = self, TMP_1), $a).call($b);
      return self.smiley = $scope.get('Smiley').$new();
    });

    Opal.defn(self, '$burger_eaten?', function(burger) {
      var self = this, distance = nil;

      distance = ($rb_plus(($rb_minus(burger.$x(), self.smiley.$x()))['$**'](2), ($rb_minus(burger.$y(), self.smiley.$y()))['$**'](2)))['$**'](0.5);
      return $rb_lt(distance, $rb_plus(40, 32));
    });

    return (Opal.defn(self, '$update', function() {
      var $a, $b, TMP_2, $c, TMP_3, self = this, dt = nil;
      if ($gvars.game == null) $gvars.game = nil;

      dt = $gvars.game.$time().$physics_elapsed();
      ($a = ($b = self.hamburgers).$each, $a.$$p = (TMP_2 = function(hamburger){var self = TMP_2.$$s || this;
if (hamburger == null) hamburger = nil;
      return hamburger.$update(dt)}, TMP_2.$$s = self, TMP_2), $a).call($b);
      if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.RIGHT)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.smiley.$move(1, 0, dt)};
      ...
      if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.DOWN)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.smiley.$move(0, 1, dt)};
      if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.UP)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.smiley.$move(0, -1, dt)};
      (($a = ["Burgers eaten: " + (self.score)]), $c = self.score_text, $c['$text='].apply($c, $a), $a[$a.length-1]);
      return ($a = ($c = self.hamburgers).$each, $a.$$p = (TMP_3 = function(burger){var self = TMP_3.$$s || this, $a;
        if (self.score == null) self.score = nil;
if (burger == null) burger = nil;
      if ((($a = self['$burger_eaten?'](burger)) !== nil && (!$a.$$is_boolean || $a == true))) {
