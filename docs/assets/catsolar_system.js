...
(function(Opal) {
  ...
  self.$require("app/catsolar_system"+ '/../' + "common");
  self.$require("app/catsolar_system"+ '/../' + "shaders/sun");
  (function($base, $super) {
    function $Cat(){};
  ...
  (function($base, $super) {
    ...
    def.filter = def.sprite = def.cat = nil;
    Opal.defn(self, '$preload', function() {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;

      return $gvars.game.$load().$image("cat", "/images/cat_images/cat4.png");
    });
    ...
    Opal.defn(self, '$create', function() {
      ...
      (($a = ["000"]), $b = $gvars.game.$stage(), $b['$background_color='].apply($b, $a), $a[$a.length-1]);
      self.filter = (($scope.get('Phaser')).$$scope.get('Filter')).$new($gvars.game, nil, $scope.get('ShaderSun'));
      self.filter.$set_resolution($gvars.size_x, $gvars.size_y);
      self.sprite = $gvars.game.$add().$sprite($rb_divide($gvars.size_x, 2), $rb_divide($gvars.size_y, 2));
      self.sprite.$anchor().$set(0.5, 0.5);
      (($a = [$rb_divide($gvars.size_x, 2)]), $b = self.sprite, $b['$width='].apply($b, $a), $a[$a.length-1]);
      (($a = [$rb_divide($gvars.size_y, 2)]), $b = self.sprite, $b['$height='].apply($b, $a), $a[$a.length-1]);
      (($a = [[self.filter]]), $b = self.sprite, $b['$filters='].apply($b, $a), $a[$a.length-1]);
      return self.cat = $scope.get('Cat').$new();
    });

    return (Opal.defn(self, '$update', function() {
      var self = this, dt = nil;
      if ($gvars.game == null) $gvars.game = nil;
