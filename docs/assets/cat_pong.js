...
(function(Opal) {
  ...
  Opal.add_stubs(['$graphics', '$add', '$/', '$begin_fill', '$draw_rect', '$<', '$abs', '$-', '$y', '$+', '$y=', '$clamp', '$math', '$sprite', '$height=', '$width=', '$set', '$anchor', '$reset!', '$*', '$cos', '$deg_to_rad', '$sin', '$**', '$x=', '$==', '$between', '$rnd', '$launch!', '$x', '$hit?', '$bounce_right_paddle!', '$value=', '$value', '$fail_effects!', '$ball_reached_left_side!', '$>', '$ball_reached_right_side!', '$-@', '$attr_reader', '$text', '$text=', '$image', '$load', '$audio', '$background_color=', '$stage', '$line_style', '$step', '$move_to', '$line_to', '$new', '$emitter', '$make_particles', '$gravity=', '$start', '$play', '$speed', '$bounce_effects!', '$bounce_left_paddle!', '$physics_elapsed', '$time', '$down?', '$keyboard', '$input', '$update', '$state']);
  self.$require("app/cat_pong"+ '/../' + "common");
  (function($base, $super) {
    function $Paddle(){};
    ...
    def.paddle = nil;
    Opal.defn(self, '$initialize', function(position) {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;

      self.paddle = $gvars.game.$add().$graphics(position, $rb_divide($gvars.size_y, 2));
      self.paddle.$begin_fill(0);
      return self.paddle.$draw_rect(0, -65, 30, 130);
    });

    Opal.defn(self, '$hit?', function(ball) {
      var self = this;

      return $rb_lt(($rb_minus(self.paddle.$y(), ball.$y())).$abs(), $rb_plus(65, 25));
    });
    ...
    return (Opal.defn(self, '$y=', function(v) {
      var $a, $b, self = this;
      if ($gvars.game == null) $gvars.game = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;

      return (($a = [$gvars.game.$math().$clamp(v, 75, $rb_minus($gvars.size_y, 75))]), $b = self.paddle, $b['$y='].apply($b, $a), $a[$a.length-1]);
    }), nil) && 'y=';
  })($scope.base, null);
  (function($base, $super) {
    function $Ball(){};
    ...
    def.ball = def.dx = def.dy = def.right_paddle = def.left_score = def.state = nil;
    Opal.defn(self, '$initialize', function(state) {
      var $a, $b, self = this;
      if ($gvars.game == null) $gvars.game = nil;

      self.ball = $gvars.game.$add().$sprite(0, 0, "cat");
      (($a = [50]), $b = self.ball, $b['$height='].apply($b, $a), $a[$a.length-1]);
      (($a = [50]), $b = self.ball, $b['$width='].apply($b, $a), $a[$a.length-1]);
      self.ball.$anchor().$set(0.5, 0.5);
      self.state = state;
      return self['$reset!']();
    });

    Opal.defn(self, '$launch!', function(speed, angle) {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;

      self.dx = $rb_times($scope.get('Math').$cos($gvars.game.$math().$deg_to_rad(angle)), speed);
      return self.dy = $rb_times($scope.get('Math').$sin($gvars.game.$math().$deg_to_rad(angle)), speed);
    });

    Opal.defn(self, '$speed', function() {
      var self = this;

      return ($rb_plus(self.dx['$**'](2), self.dy['$**'](2)))['$**'](0.5);
    });

    Opal.defn(self, '$reset!', function() {
      var $a, $b, self = this, angle = nil;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;
      if ($gvars.game == null) $gvars.game = nil;

      (($a = [$rb_divide($gvars.size_x, 2)]), $b = self.ball, $b['$x='].apply($b, $a), $a[$a.length-1]);
      (($a = [$rb_divide($gvars.size_y, 2)]), $b = self.ball, $b['$y='].apply($b, $a), $a[$a.length-1]);
      if ($gvars.game.$rnd().$between(0, 1)['$=='](0)) {
        angle = $gvars.game.$rnd().$between(-45, 45)
        } else {
        angle = $gvars.game.$rnd().$between($rb_minus(180, 45), $rb_plus(180, 45))
      };
      return self['$launch!'](300.0, angle);
    });
    ...
    Opal.defn(self, '$ball_reached_right_side!', function() {
      var $a, self = this;

      if ((($a = self.right_paddle['$hit?'](self.ball)) !== nil && (!$a.$$is_boolean || $a == true))) {
        return self['$bounce_right_paddle!']()
        } else {
        ($a = self.left_score, $a['$value=']($rb_plus($a.$value(), 1)));
        return self['$fail_effects!']();
      };
    });
    ...
    return (Opal.defn(self, '$update', function(dt) {
      ...
      ($a = self.ball, $a['$x=']($rb_plus($a.$x(), $rb_times(dt, self.dx))));
      ($a = self.ball, $a['$y=']($rb_plus($a.$y(), $rb_times(dt, self.dy))));
      if ((($a = ($b = $rb_lt(self.ball.$x(), 65), $b !== false && $b !== nil ?$rb_lt(self.dx, 0) : $b)) !== nil && (!$a.$$is_boolean || $a == true))) {
        return self.state['$ball_reached_left_side!']()
      } else if ((($a = ($b = $rb_gt(self.ball.$x(), $rb_minus($gvars.size_x, 65)), $b !== false && $b !== nil ?$rb_gt(self.dx, 0) : $b)) !== nil && (!$a.$$is_boolean || $a == true))) {
        return self.state['$ball_reached_right_side!']()
      } else if ((($a = ($b = $rb_lt(self.ball.$y(), 25), $b !== false && $b !== nil ?$rb_lt(self.dy, 0) : $b)) !== nil && (!$a.$$is_boolean || $a == true))) {
        return self.dy = self.dy['$-@']()
      } else if ((($a = ($b = $rb_gt(self.ball.$y(), $rb_minus($gvars.size_y, 25)), $b !== false && $b !== nil ?$rb_gt(self.dy, 0) : $b)) !== nil && (!$a.$$is_boolean || $a == true))) {
        return self.dy = self.dy['$-@']()
        } else {
        return nil
      };
  ...
  (function($base, $super) {
    ...
    var self = $ScoreDisplay = $klass($base, $super, 'ScoreDisplay', $ScoreDisplay);
    ...
    Opal.defn(self, '$initialize', function(x, y, v) {
      var $a, $b, self = this;
      if ($gvars.game == null) $gvars.game = nil;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;
      ...
      self.label = $gvars.game.$add().$text($rb_times(x, $gvars.size_x), $rb_times(y, $gvars.size_y), "", $hash2(["fontSize", "fill", "align"], {"fontSize": "100px", "fill": "#000", "align": "center"}));
      self.label.$anchor().$set(0.5);
      return (($a = [v]), $b = self, $b['$value='].apply($b, $a), $a[$a.length-1]);
    });
    ...
    return (Opal.defn(self, '$value=', function(v) {
      ...
      self.value = v;
      return (($a = [self.value]), $b = self.label, $b['$text='].apply($b, $a), $a[$a.length-1]);
    }), nil) && 'value=';
  })($scope.base, null);
  (function($base, $super) {
    function $MainState(){};
    ...
    def.grid = def.emitter = def.ball = def.meow = def.meow2 = def.left_paddle = def.right_paddle = def.right_score = def.left_score = nil;
    Opal.defn(self, '$preload', function() {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;

      $gvars.game.$load().$image("cat", "/images/cat_images/cat17.png");
      $gvars.game.$load().$image("star", "/images/star-icon.png");
      $gvars.game.$load().$audio("meow", "/audio/cat_meow.mp3");
      return $gvars.game.$load().$audio("meow2", "/audio/cat_meow_2.mp3");
    });
    ...
    Opal.defn(self, '$create', function() {
      ...
      (($a = ["AAFFAA"]), $b = $gvars.game.$stage(), $b['$background_color='].apply($b, $a), $a[$a.length-1]);
      self.grid = $gvars.game.$add().$graphics($rb_divide($gvars.size_x, 2), $rb_divide($gvars.size_y, 2));
      self.grid.$line_style(5, "white");
      ($a = ($b = ($range($rb_divide($gvars.size_y['$-@'](), 2), $rb_divide($gvars.size_y, 2), false))).$step, $a.$$p = (TMP_1 = function(y){var self = TMP_1.$$s || this;
        if (self.grid == null) self.grid = nil;
if (y == null) y = nil;
      self.grid.$move_to(0, y);
        return self.grid.$line_to(0, $rb_plus(y, 10));}, TMP_1.$$s = self, TMP_1), $a).call($b, 20);
      self.left_score = $scope.get('ScoreDisplay').$new(0.25, 0.125);
      self.right_score = $scope.get('ScoreDisplay').$new(0.75, 0.125);
      self.left_paddle = $scope.get('Paddle').$new(10);
      self.right_paddle = $scope.get('Paddle').$new($rb_minus($gvars.size_x, 40));
      self.emitter = $gvars.game.$add().$emitter(0, 0, 1000);
      self.emitter.$make_particles("star");
      (($a = [200]), $c = self.emitter, $c['$gravity='].apply($c, $a), $a[$a.length-1]);
      self.ball = $scope.get('Ball').$new(self);
      self.meow = $gvars.game.$add().$audio("meow");
      return self.meow2 = $gvars.game.$add().$audio("meow2");
    });

    Opal.defn(self, '$bounce_effects!', function() {
      var $a, $b, self = this;

      (($a = [self.ball.$x()]), $b = self.emitter, $b['$x='].apply($b, $a), $a[$a.length-1]);
      (($a = [self.ball.$y()]), $b = self.emitter, $b['$y='].apply($b, $a), $a[$a.length-1]);
      self.emitter.$start(true, 5000, nil, 10);
      return self.meow.$play();
    });

    Opal.defn(self, '$fail_effects!', function() {
      var self = this;

      self.meow2.$play();
      return self.ball['$reset!']();
    });

    Opal.defn(self, '$bounce_left_paddle!', function() {
      var self = this, intercept = nil, speed = nil;

      intercept = $rb_divide(($rb_minus(self.left_paddle.$y(), self.ball.$y())), ($rb_plus(65, 25)));
      speed = $rb_times(1.1, self.ball.$speed());
      self.ball['$launch!'](speed, $rb_minus(0, $rb_times(45, intercept)));
      return self['$bounce_effects!']();
    });

    Opal.defn(self, '$bounce_right_paddle!', function() {
      var self = this, intercept = nil, speed = nil;

      intercept = $rb_divide(($rb_minus(self.right_paddle.$y(), self.ball.$y())), ($rb_plus(65, 25)));
      speed = $rb_times(1.1, self.ball.$speed());
      self.ball['$launch!'](speed, $rb_plus(180, $rb_times(45, intercept)));
      return self['$bounce_effects!']();
    });

    Opal.defn(self, '$ball_reached_left_side!', function() {
      var $a, self = this;

      if ((($a = self.left_paddle['$hit?'](self.ball)) !== nil && (!$a.$$is_boolean || $a == true))) {
        return self['$bounce_left_paddle!']()
        } else {
        ($a = self.right_score, $a['$value=']($rb_plus($a.$value(), 1)));
        return self['$fail_effects!']();
      };
    });
    ...
    return (Opal.defn(self, '$update', function() {
      var $a, self = this, dt = nil;
      if ($gvars.game == null) $gvars.game = nil;

      dt = $gvars.game.$time().$physics_elapsed();
      if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.W)) !== nil && (!$a.$$is_boolean || $a == true))) {
        ($a = self.left_paddle, $a['$y=']($rb_minus($a.$y(), $rb_times(dt, 600))))
      } else if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.S)) !== nil && (!$a.$$is_boolean || $a == true))) {
        ($a = self.left_paddle, $a['$y=']($rb_plus($a.$y(), $rb_times(dt, 600))))};
      if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.UP)) !== nil && (!$a.$$is_boolean || $a == true))) {
        ($a = self.right_paddle, $a['$y=']($rb_minus($a.$y(), $rb_times(dt, 600))))
      } else if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.DOWN)) !== nil && (!$a.$$is_boolean || $a == true))) {
        ($a = self.right_paddle, $a['$y=']($rb_plus($a.$y(), $rb_times(dt, 600))))};
