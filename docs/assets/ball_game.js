...
(function(Opal) {
  ...
  Opal.add_stubs(['$ensure_bounds', '$random_direction', '$graphics', '$add', '$begin_fill', '$rand', '$draw_circle', '$deg_to_rad', '$math', '$between', '$rnd', '$*', '$cos', '$sin', '$clamp', '$-', '$!=', '$-@', '$+', '$x=', '$y=', '$background_color=', '$stage', '$on', '$<<', '$new', '$input', '$physics_elapsed', '$time', '$each', '$update', '$state']);
  self.$require("app/ball_game"+ '/../' + "common");
  (function($base, $super) {
    function $Ball(){};
    ...
    Opal.defn(self, '$initialize', function(x, y) {
      ...
      self.graphics.$begin_fill($scope.get('Math').$rand(16777216));
      return self.graphics.$draw_circle(0, 0, 10);
    });

    Opal.defn(self, '$random_direction', function() {
      var self = this, angle = nil, speed = nil;
      if ($gvars.game == null) $gvars.game = nil;

      angle = $gvars.game.$math().$deg_to_rad($gvars.game.$rnd().$between(0, 360));
      speed = $gvars.game.$rnd().$between(200, 600);
      self.dx = $rb_times($scope.get('Math').$cos(angle), speed);
      return self.dy = $rb_times($scope.get('Math').$sin(angle), speed);
    });

    Opal.defn(self, '$ensure_bounds', function() {
      var $a, self = this, new_x = nil, new_y = nil;
      if ($gvars.game == null) $gvars.game = nil;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;

      new_x = $gvars.game.$math().$clamp(self.x, 10, $rb_minus($gvars.size_x, 10));
      new_y = $gvars.game.$math().$clamp(self.y, 10, $rb_minus($gvars.size_y, 10));
      if ((($a = [self.x, self.y]['$!=']([new_x, new_y])) !== nil && (!$a.$$is_boolean || $a == true))) {
        if ((($a = new_x['$!='](self.x)) !== nil && (!$a.$$is_boolean || $a == true))) {
          self.dx = self.dx['$-@']()};
        if ((($a = new_y['$!='](self.y)) !== nil && (!$a.$$is_boolean || $a == true))) {
          self.dy = self.dy['$-@']()};
        $a = [new_x, new_y], self.x = $a[0], self.y = $a[1], $a;
        self.dx = $rb_times(self.dx, 0.8);
        return self.dy = $rb_times(self.dy, 0.8);
        } else {
        return nil
      };
    });
    ...
    return (Opal.defn(self, '$update', function(dt) {
      ...
      self.dy = $rb_plus(self.dy, $rb_times(20.0, dt));
      ...
      return (($a = [self.y]), $b = self.graphics, $b['$y='].apply($b, $a), $a[$a.length-1]);
    }), nil) && 'update';
  })($scope.base, null);
  (function($base, $super) {
    function $MainState(){};
    ...
    Opal.defn(self, '$create', function() {
      ...
      return ($a = ($b = $gvars.game.$input()).$on, $a.$$p = (TMP_1 = function(pointer, event){var self = TMP_1.$$s || this;
        if (self.balls == null) self.balls = nil;
if (pointer == null) pointer = nil;if (event == null) event = nil;
      return self.balls['$<<']($scope.get('Ball').$new(pointer.x, pointer.y))}, TMP_1.$$s = self, TMP_1), $a).call($b, "down");
    });
    ...
    return (Opal.defn(self, '$update', function() {
      ...
      return ($a = ($b = self.balls).$each, $a.$$p = (TMP_2 = function(ball){var self = TMP_2.$$s || this;
if (ball == null) ball = nil;
      return ball.$update(dt)}, TMP_2.$$s = self, TMP_2), $a).call($b);
    }), nil) && 'update';
