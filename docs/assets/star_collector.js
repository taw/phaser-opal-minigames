...
(function(Opal) {
  ...
  Opal.add_stubs(['$attr_reader', '$/', '$graphics', '$add', '$begin_fill', '$draw_polygon', '$<', '$-@', '$>', '$-', '$+', '$*', '$x=', '$y=', '$angle=', '$ensure_bounds', '$sin', '$deg_to_rad', '$math', '$cos', '$**', '$dir_x', '$dir_y', '$limit_speed', '$sprite', '$set', '$anchor', '$between', '$rnd', '$angle', '$image', '$load', '$text', '$background_color=', '$stage', '$new', '$map', '$tap', '$relocate_star', '$times', '$x', '$launch', '$physics_elapsed', '$time', '$down?', '$keyboard', '$input', '$speed_up', '$turn', '$text=', '$update', '$each', '$collision?', '$y', '$sqrt', '$state']);
  self.$require("app/star_collector"+ '/../' + "common");
  (function($base, $super) {
    function $SpaceShip(){};
    var self = $SpaceShip = $klass($base, $super, 'SpaceShip', $SpaceShip);

    var def = self.$$proto, $scope = self.$$scope;

    def.x = def.y = def.graphics = def.dx = def.dy = def.angle = nil;
    ...
    Opal.defn(self, '$initialize', function() {
      ...
      self.x = $rb_divide($gvars.size_x, 2);
      self.y = $rb_divide($gvars.size_y, 2);
      self.angle = 0;
      self.dx = 0;
      self.dy = 0;
      self.graphics = $gvars.game.$add().$graphics(self.x, self.y);
      self.graphics.$begin_fill(65280);
      return self.graphics.$draw_polygon([0, -20, 15, 10, 15, 20, -15, 20, -15, 10]);
    });

    Opal.defn(self, '$ensure_bounds', function() {
      var $a, $b, self = this;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;

      if ((($a = ($b = $rb_lt(self.x, 20), $b !== false && $b !== nil ?$rb_lt(self.dx, 0) : $b)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.dx = self.dx['$-@']()};
      if ((($a = ($b = $rb_gt(self.x, $rb_minus($gvars.size_x, 20)), $b !== false && $b !== nil ?$rb_gt(self.dx, 0) : $b)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.dx = self.dx['$-@']()};
      if ((($a = ($b = $rb_lt(self.y, 20), $b !== false && $b !== nil ?$rb_lt(self.dy, 0) : $b)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.dy = self.dy['$-@']()};
      if ((($a = ($b = $rb_gt(self.y, $rb_minus($gvars.size_y, 20)), $b !== false && $b !== nil ?$rb_gt(self.dy, 0) : $b)) !== nil && (!$a.$$is_boolean || $a == true))) {
    ...
    Opal.defn(self, '$update', function(dt) {
      var $a, $b, self = this;

      self.x = $rb_plus(self.x, $rb_times(self.dx, dt));
      self.y = $rb_plus(self.y, $rb_times(self.dy, dt));
      (($a = [self.x]), $b = self.graphics, $b['$x='].apply($b, $a), $a[$a.length-1]);
      (($a = [self.y]), $b = self.graphics, $b['$y='].apply($b, $a), $a[$a.length-1]);
      (($a = [self.angle]), $b = self.graphics, $b['$angle='].apply($b, $a), $a[$a.length-1]);
      return self.$ensure_bounds();
    });

    Opal.defn(self, '$dir_x', function() {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;

      return $scope.get('Math').$sin($gvars.game.$math().$deg_to_rad(self.angle));
    });

    Opal.defn(self, '$dir_y', function() {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;

      return $scope.get('Math').$cos($gvars.game.$math().$deg_to_rad(self.angle))['$-@']();
    });

    Opal.defn(self, '$limit_speed', function() {
      var $a, self = this, dl = nil, max_speed = nil;

      dl = ($rb_plus($rb_times(self.dx, self.dx), $rb_times(self.dy, self.dy)))['$**'](0.5);
      max_speed = 400.0;
      if ((($a = $rb_gt(dl, max_speed)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.dx = $rb_times(self.dx, ($rb_divide(max_speed, dl)));
        return self.dy = $rb_times(self.dy, ($rb_divide(max_speed, dl)));
        } else {
        return nil
      };
    });

    Opal.defn(self, '$speed_up', function(dir, dt) {
      var self = this, speed_gain_per_second = nil;

      speed_gain_per_second = 200.0;
      self.dx = $rb_plus(self.dx, $rb_times($rb_times($rb_times(self.$dir_x(), dt), speed_gain_per_second), dir));
      self.dy = $rb_plus(self.dy, $rb_times($rb_times($rb_times(self.$dir_y(), dt), speed_gain_per_second), dir));
      return self.$limit_speed();
    });

    return (Opal.defn(self, '$turn', function(dir, dt) {
      var self = this, degrees_per_second = nil;

      degrees_per_second = 400.0;
      return self.angle = $rb_plus(self.angle, $rb_times($rb_times(dir, dt), degrees_per_second));
    }), nil) && 'turn';
  })($scope.base, null);
  (function($base, $super) {
    function $Star(){};
    ...
    def.x = def.y = def.graphics = def.dx = def.dy = nil;
    self.$attr_reader("x", "y");
    ...
    Opal.defn(self, '$initialize', function() {
      ...
      self.graphics = $gvars.game.$add().$sprite(self.x, self.y, "star");
      return self.graphics.$anchor().$set(0.5, 0.5);
    });

    Opal.defn(self, '$launch', function(x, y) {
      var self = this, angle = nil, speed = nil;
      if ($gvars.game == null) $gvars.game = nil;

      self.x = x;
      self.y = y;
      angle = $gvars.game.$rnd().$between(0, 360);
      speed = $gvars.game.$rnd().$between(50, 200);
    ...
    Opal.defn(self, '$update', function(dt) {
      ...
      (($a = [self.x]), $b = self.graphics, $b['$x='].apply($b, $a), $a[$a.length-1]);
      (($a = [self.y]), $b = self.graphics, $b['$y='].apply($b, $a), $a[$a.length-1]);
      ($a = self.graphics, $a['$angle=']($rb_plus($a.$angle(), $rb_times(100, dt))));
      return self.$ensure_bounds();
    });

    return (Opal.defn(self, '$ensure_bounds', function() {
      var $a, $b, self = this;
      if ($gvars.size_x == null) $gvars.size_x = nil;
      if ($gvars.size_y == null) $gvars.size_y = nil;
  ...
  (function($base, $super) {
    ...
    def.space_ship = def.score = def.score_text = def.stars = nil;
    Opal.defn(self, '$preload', function() {
      var self = this;
      if ($gvars.game == null) $gvars.game = nil;
    ...
    Opal.defn(self, '$create', function() {
      ...
      self.score = 0.0;
      self.score_text = $gvars.game.$add().$text(10, 10, "", $hash2(["fontSize", "fill", "align"], {"fontSize": "16px", "fill": "#000", "align": "center"}));
      (($a = ["448"]), $b = $gvars.game.$stage(), $b['$background_color='].apply($b, $a), $a[$a.length-1]);
      self.space_ship = $scope.get('SpaceShip').$new();
      return self.stars = ($a = ($b = (25).$times()).$map, $a.$$p = (TMP_1 = function(){var self = TMP_1.$$s || this, $a, $b, TMP_2;

      return ($a = ($b = $scope.get('Star').$new()).$tap, $a.$$p = (TMP_2 = function(s){var self = TMP_2.$$s || this;
if (s == null) s = nil;
        return self.$relocate_star(s)}, TMP_2.$$s = self, TMP_2), $a).call($b)}, TMP_1.$$s = self, TMP_1), $a).call($b);
    });

    Opal.defn(self, '$relocate_star', function(star) {
      ...
      x = $gvars.game.$rnd().$between(100, $rb_minus($gvars.size_x, 100));
      y = $gvars.game.$rnd().$between(100, $rb_minus($gvars.size_y, 100));
      if ((($b = $rb_gt($rb_plus(($rb_minus(x, self.space_ship.$x()))['$**'](2), ($rb_minus(y, self.space_ship.$x()))['$**'](2)), (200)['$**'](2))) !== nil && (!$b.$$is_boolean || $b == true))) {
        break;};};
      return star.$launch(x, y);
    });

    Opal.defn(self, '$update', function() {
      var $a, $b, TMP_3, self = this, dt = nil;
      if ($gvars.game == null) $gvars.game = nil;

      dt = $gvars.game.$time().$physics_elapsed();
      if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.UP)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.space_ship.$speed_up(1.0, dt)};
      if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.DOWN)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.space_ship.$speed_up(-1.0, dt)};
      if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.LEFT)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.space_ship.$turn(-1.0, dt)};
      if ((($a = $gvars.game.$input().$keyboard()['$down?'](Phaser.KeyCode.RIGHT)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.space_ship.$turn(1.0, dt)};
      (($a = ["Stars Collected: " + (self.score)]), $b = self.score_text, $b['$text='].apply($b, $a), $a[$a.length-1]);
      self.space_ship.$update(dt);
      return ($a = ($b = self.stars).$each, $a.$$p = (TMP_3 = function(star){var self = TMP_3.$$s || this, $a;
        if (self.space_ship == null) self.space_ship = nil;
        if (self.score == null) self.score = nil;
if (star == null) star = nil;
      star.$update(dt);
        if ((($a = self['$collision?'](self.space_ship, star)) !== nil && (!$a.$$is_boolean || $a == true))) {
          self.score = $rb_plus(self.score, 1);
          return self.$relocate_star(star);
          } else {
          return nil
        };}, TMP_3.$$s = self, TMP_3), $a).call($b);
    });

    return (Opal.defn(self, '$collision?', function(a, b) {
      var self = this, dx = nil, dy = nil;

      dx = $rb_minus(a.$x(), b.$x());
      dy = $rb_minus(a.$y(), b.$y());
      return $rb_lt($scope.get('Math').$sqrt($rb_plus(($rb_times(dx, dx)), ($rb_times(dy, dy)))), 25);
    }), nil) && 'collision?';
